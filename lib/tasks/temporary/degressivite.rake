...
namespace :change do
  ...
  # ====================== Code ajoute pour la correction de la degressivite : il faut mettre à jour le code de la fonction ci-dessus =====
  # organizations = Organization.where(name: ["CDS MATERIEL", "07_DSI Matériel_CdS Matériel"]).all
  # organizations.each do |o|
  #   Guw::GuwComplexityCoefficientElement.where(organization_id: o.id, guw_complexity_id: nil).delete_all
  #   Guw::GuwComplexityCoefficientElement.where(organization_id: o.id, value: nil).delete_all
  #
  #   p " **** =========> #{o.name} <========== **** "
  #
  #   o.guw_models.each do |guw_model|
