  ...
  def initialize(user, organization, projects, nb_project = 1, estimation_view = true)
    ...
    if !organization.nil?
      ...
      if user && !user_groups.empty?
        ...
        pe = Permission.where(id: status_global.map{|i| i[0]}).all
        pp = Project.where(id: status_global.map{|i| i[1]}).all
        ss = EstimationStatus.where(id: status_global.map{|i| i[2]}).all
