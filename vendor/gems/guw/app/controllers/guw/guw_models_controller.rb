        ...
        if Guw::GuwCoefficient.where(organization_id: organization_id,
          ...
          unless guw_coefficient.nil?
            unless guw_coefficient.guw_coefficient_elements.empty?
              ...
              if guw_coefficient.coefficient_type == "Pourcentage"
                @worksheet.add_cell(ind, 22+j, (ceuw.nil? ? 100 : ceuw.percent.to_f).to_s)
              elsif guw_coefficient.coefficient_type == "Coefficient"
                @worksheet.add_cell(ind, 22+j, (ceuw.nil? ? 100 : ceuw.percent.to_f).to_s)
              else
                @worksheet.add_cell(ind, 22+j, ceuw.nil? ? '' : ceuw.guw_coefficient_element.nil? ? ceuw.percent : ceuw.guw_coefficient_element.name)
              end

              @worksheet.add_dropdown(ind, 22+j, nil, 'Commentaire', ceuw.nil? ? '' : ceuw.comments)
          ...
          unless guow.guw_type.nil?
            unless guw_output.nil?
              ...
              @worksheet.add_cell(ind, 22+j, v.to_s)
            end
          end
        end
      end
