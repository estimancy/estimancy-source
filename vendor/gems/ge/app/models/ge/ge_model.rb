...
module Ge
  class GeModel < ActiveRecord::Base
    ...
    def self.display_value(data_probable, estimation_value, view_widget, user)
      ...
      return "#{ActionController::Base.helpers.number_with_precision(result_value, delimiter: delimiter, precision: precision, separator: separator, locale: (user.language.locale rescue "fr"))} #{unit}"
    end
